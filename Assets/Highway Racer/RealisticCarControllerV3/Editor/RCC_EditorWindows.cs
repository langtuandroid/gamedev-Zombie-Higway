...
public class RCC_EditorWindows : Editor {
	...
	public static void CreateRCCCamera(){
		...
		if (GameObject.FindObjectOfType<CarCameraController> ()) {
			...
			Selection.activeGameObject = GameObject.FindObjectOfType<CarCameraController> ().gameObject;

		} else {

			GameObject cam = Instantiate (RCC_Settings.Instance.rccMainCameraController.gameObject);
			cam.name = RCC_Settings.Instance.rccMainCameraController.name;
