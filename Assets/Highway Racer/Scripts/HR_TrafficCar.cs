...
public class HR_TrafficCar : MonoBehaviour {
	...
	#region GameHandler Instance

	private GameHandler _gameHandlerInstance;
	private GameHandler GameHandler {
		get {
			if (_gameHandlerInstance == null) {
				_gameHandlerInstance = GameHandler.Instance;
			}
			return _gameHandlerInstance;
		}
	}

	#endregion

	#region TrafficManager Instance

	private TrafficManager _trafficManagerInstance;
	private TrafficManager TrafficManager {
		get {
			if (_trafficManagerInstance == null) {
				_trafficManagerInstance = TrafficManager.Instance;
			}
			return _trafficManagerInstance;
		}
	}
	...
	void Awake () {
		...
		if(GameHandler.dayOrNight == GameHandler.DayOrNight.Night || GameHandler.dayOrNight == GameHandler.DayOrNight.Rainy)
			headlightsOn = true;
		else
			headlightsOn = false;
	...
	void FixedUpdate(){
		...
		if(!immobilized && GameHandler.mode != GameHandler.Mode.TwoWay)
		...
		if(!immobilized && GameHandler.mode != GameHandler.Mode.TwoWay){
			...
			case ChangingLines.Left:
				...
				if(transform.position.x <= TrafficManager.lines[currentLine - 1].position.x + .5f){
					currentLine --;
			...
			case ChangingLines.Right:
				...
				if(currentLine == (TrafficManager.lines.Length - 1)){
					changingLines = ChangingLines.Straight;
					break;
				}
