...
public class CarCameraController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
            ...
            else
            {
                if (Time.timeScale >= 1)
                    transform.position = new Vector3(transform.position.x, transform.position.y,
                        transform.position.z + Mathf.Clamp(currentT, 0f, Mathf.Infinity));
            }
        ...
        audioListener.transform.position =
            new Vector3(playerCar.position.x, transform.position.y, transform.position.z);
    ...
    {
        transform.rotation =
            Quaternion.Lerp(transform.rotation, Quaternion.Euler(rotation, 0f, 0f), Time.deltaTime * 2f);
        ...
        transform.position = SmoothApproach(pastFollowerPosition, pastTargetPosition, targetPosition,
            (speed / 10f) * Mathf.Clamp(Time.timeSinceLevelLoad - 1.5f, 0f, .85f));
    ...
    {
        transform.rotation =
            Quaternion.Lerp(transform.rotation, Quaternion.Euler(rotation / 4f, 0f, 0f), Time.deltaTime * 2f);
        targetPosition = new Vector3(playerCar.position.x, tpsCam.transform.position.y, tpsCam.transform.position.z);
        transform.position = SmoothApproach(pastFollowerPosition, pastTargetPosition, targetPosition,
            (speed / 3f) * Mathf.Clamp(Time.timeSinceLevelLoad - 1.5f, 0f, .85f));
    ...
    void FPS()
    {
        ...
        if (HR_HighwayRacerProperties.Instance._tiltCamera)
            transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y,
                -transform.InverseTransformDirection(playerRigid.velocity).x / 2f);
