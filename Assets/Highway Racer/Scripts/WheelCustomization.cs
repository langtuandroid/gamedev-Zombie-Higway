...
public class WheelCustomization : MonoBehaviour
{
    public int wheelIndex;

    public int wheelPrice
    {
        get { return WheelCollection.Instance.wheels[wheelIndex].price; }
    }
    ...
    void Start()
    {
        ...
        unlocked = WheelCollection.Instance.wheels[wheelIndex].unlocked;
    }
    ...
    void Update()
    {
        ...
        {
            PlayerPrefs.SetInt(
                HR_MainMenuHandler.Instance.currentCar.transform.name + "OwnedWheel" + currentWheelString, 1);
            unlocked = true;
        }

        if (PlayerPrefs.HasKey(
                HR_MainMenuHandler.Instance.currentCar.transform.name + "OwnedWheel" + currentWheelString) ||
            WheelCollection.Instance.wheels[wheelIndex].unlocked)
            unlocked = true;
        else
            unlocked = false;
